...
export function PartnerStatsContent({ stats, isBazar }: { stats: StatsData; isBazar: boolean }) {
      ...
      {isBazar && stats?.funnel ? (
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            <StatCard icon="🚗" iconColor="blue" value={String(stats.totalVehicles || 0)} label="Celkem vozidel" />
            <StatCard icon="✅" iconColor="green" value={String(stats.activeVehicles || 0)} label="Aktivnich" />
          </div>
        </>
      ) : stats ? (
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          <StatCard icon="🔧" iconColor="blue" value={String(stats.totalParts || 0)} label="Celkem dilu" />
          <StatCard icon="✅" iconColor="green" value={String(stats.activeParts || 0)} label="Aktivnich" />
          <StatCard icon="📦" iconColor="orange" value={String(stats.totalOrders || 0)} label="Objednavek" />
        </div>
      ) : null}
      ...
      {!chartLoading && chartData.length > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
          ...
          <Card className="p-4">
            ...
            <OrdersChart data={chartData} height={180} barLabel={isBazar ? "Prodeje" : "Objednavky"} />
          </Card>
        </div>
      )}
