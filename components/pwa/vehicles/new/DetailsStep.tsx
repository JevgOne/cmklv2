...
export function DetailsStep() {
  ...
  // Auto-generate description when equipment >= 3 and no description yet
  useEffect(() => {
    if (autoGenTriggered.current) return;
    if (generatingDescription) return;
    if (form.description && form.description.length > 0) return;
    if (!form.equipment || form.equipment.length < 3) return;
  ...
  }, [form.equipment, form.description, form.brand, form.model, generatingDescription, handleGenerateDescription]);
      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <div className="mt-4 space-y-3">
            ...
            />
            {autoGenerated && form.description && (
              <div className="flex items-center gap-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 shrink-0">
                  <path fillRule="evenodd" d="M9 4.5a.75.75 0 01.721.544l.813 2.846a3.75 3.75 0 002.576 2.576l2.846.813a.75.75 0 010 1.442l-2.846.813a3.75 3.75 0 00-2.576 2.576l-.813 2.846a.75.75 0 01-1.442 0l-.813-2.846a3.75 3.75 0 00-2.576-2.576l-2.846-.813a.75.75 0 010-1.442l2.846-.813A3.75 3.75 0 007.466 7.89l.813-2.846A.75.75 0 019 4.5z" clipRule="evenodd" />
                </svg>
                <span>Popis byl automaticky vygenerován z výbavy. Kliknutím do pole ho upravte.</span>
              </div>
            )}
            <Button
              variant="primary"
              size="lg"
              onClick={() => { setAutoGenerated(false); handleGenerateDescription(); }}
              ...
              {generatingDescription ? (
                ...
                <div className="flex flex-col items-center gap-0.5">
                  ...
                  <span className="text-xs font-normal opacity-80">
                    Automaticky vytvoří prodejní popis z výbavy a technických dat
                  </span>
                </div>
              )}
            </Button>
