...
export function ProfileEditor({ initialData, initialTags }: ProfileEditorProps) {
  const [data, setData] = useState<ProfileEditData>(initialData);
  ...
  const [favBrands, setFavBrands] = useState<string[]>(() => {
    if (!initialData.favoriteBrands) return [];
    try { return JSON.parse(initialData.favoriteBrands); } catch { return []; }
  });
  ...
  const [yearsExperience, setYearsExperience] = useState<number | "">(initialData.yearsExperience ?? "");
  const [website, setWebsite] = useState(initialData.website || "");
  const [motto, setMotto] = useState(initialData.motto || "");
  const [socialInstagram, setSocialInstagram] = useState(initialData.socialLinks?.instagram || "");
  const [socialFacebook, setSocialFacebook] = useState(initialData.socialLinks?.facebook || "");
  const [socialYoutube, setSocialYoutube] = useState(initialData.socialLinks?.youtube || "");
  const [services, setServices] = useState<string[]>(initialData.services || []);
  const [languageSkills, setLanguageSkills] = useState<string[]>(initialData.languageSkills || []);
  const [specializations, setSpecializations] = useState<string[]>(() => {
    if (!initialData.specializations) return [];
    try { return JSON.parse(initialData.specializations); } catch { return []; }
  });
  const [warehouseAddress, setWarehouseAddress] = useState(initialData.warehouseAddress || "");
  const [openingHours, setOpeningHours] = useState<Record<string, string>>(initialData.openingHours || {});
