...
export function AdminReturnDetailContent({ initialData, userRole }: Props) {
  const router = useRouter();
  const canEdit = userRole === "ADMIN" || userRole === "BACKOFFICE";

  const [returnData, setReturnData] = useState<ReturnDetail>(initialData);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  const [status, setStatus] = useState(initialData.status);
  const [approvedAmount, setApprovedAmount] = useState(initialData.approvedAmount?.toString() || "");
  ...
  const fetchReturn = useCallback(async () => {
    try {
      const res = await fetch(`/api/admin/returns/${returnData.id}`);
              ...
              <div className="grid grid-cols-2 gap-3">
                {parsedPhotos.map((url, i) => (
                  <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="block">
                    <img src={url} alt={`Foto závady ${i + 1}`} className="rounded-lg object-cover w-full h-32 border border-gray-200" />
                  </a>
                ))}
              </div>
            </Card>
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={status} onChange={(e) => setStatus(e.target.value)} disabled={!canEdit} className="w-full h-10 px-3 rounded-lg border border-gray-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 disabled:bg-gray-100">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Schválená částka (Kč)</label>
                <input type="number" value={approvedAmount} onChange={(e) => setApprovedAmount(e.target.value)} disabled={!canEdit} placeholder={`Max ${returnData.requestedAmount} Kč`} min={0} max={returnData.requestedAmount} className="w-full h-10 px-3 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 disabled:bg-gray-100" />
              </div>

              {(status === "REJECTED" || rejectionReason) && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Důvod zamítnutí</label>
                  <textarea value={rejectionReason} onChange={(e) => setRejectionReason(e.target.value)} disabled={!canEdit} rows={3} className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 disabled:bg-gray-100 resize-none" />
                </div>
              )}

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Interní poznámky</label>
                <textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} disabled={!canEdit} rows={4} placeholder="Poznámky viditelné pouze pro admin tým..." className="w-full px-3 py-2 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 disabled:bg-gray-100 resize-none" />
              </div>
