...
export const alt = "Autodíly — CarMakléř";
...
export default async function Image({ params }: { params: Promise<{ brand: string }> }) {
  ...
  const name = brand.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");
    ...
    (
      <OgLayout logo={logo}>
        ...
        <div style={{ display: "flex", fontSize: 22, color: "rgba(255,255,255,0.7)", marginTop: 20, textAlign: "center" }}>
          Originální i aftermarket díly od ověřených dodavatelů
        </div>
      </OgLayout>
    ),
    options,
