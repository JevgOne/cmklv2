...
export const alt = "Autoservisy — CarMakléř";
export const size = OG_SIZE;
export const contentType = "image/png";

export default async function Image({ params }: { params: Promise<{ kraj: string }> }) {
  const { kraj } = await params;
  const logo = await getLogoBase64();
  const options = await ogImageOptions();

  const name = kraj.split("-").map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(" ");

  return new ImageResponse(
    (
      <OgLayout logo={logo}>
        <div style={{ display: "flex", fontSize: 48, fontWeight: 800, color: "white", textAlign: "center", lineHeight: 1.2 }}>
          <span>Autoservisy </span>
        ...
        <div style={{ display: "flex", fontSize: 22, color: "rgba(255,255,255,0.7)", marginTop: 20, textAlign: "center" }}>
          {`Ověřené autoservisy v kraji ${name}`}
        </div>
      </OgLayout>
    ),
    options,
