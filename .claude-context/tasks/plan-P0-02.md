# Plan P0-02: Pravni stranky — Ochrana osobnich udaju (GDPR)
...
Vytvorit stranku `/ochrana-osobnich-udaju` s kompletni informaci o zpracovani osobnich udaju dle GDPR (Narizeni EU 2016/679) a zakona 110/2019 Sb.
...
**Soubor:** `app/(web)/ochrana-osobnich-udaju/page.tsx` (NOVY)
...
Shodne s P0-01 — vytvorit `loading.tsx` (animate-pulse skeleton) a `error.tsx` ("use client" + fallback text).

**Soubory:**
- `app/(web)/ochrana-osobnich-udaju/loading.tsx` — viz P0-01 vzor
- `app/(web)/ochrana-osobnich-udaju/error.tsx` — viz P0-01 vzor
...
- Shodne s P0-01: Outfit font, `prose prose-gray`, `text-orange-500` linky, Breadcrumbs komponenta
- Tabulka ucelu pouziva Tailwind utility classes (`overflow-x-auto`, `min-w-full`) — responzivni na mobilu
- GDPR kontakt pouziva odlisny e-mail `gdpr@carmakler.cz` (standardni praxe)
- Interni odkazy na /zasady-cookies pouzivaji `<Link>`

---

## Dulezite poznamky

- UOOU kontakt je realny a aktualni (Pplk. Sochorova 27, Praha 7)
- Placeholder `[DOPLNIT]` oznacuji mista vyzadujici realne firemni udaje
- Po P0-04 (Cookie consent) bude stranka /zasady-cookies existovat — do te doby link vede na 404
- Text MUSI byt revidovany pravnikem/DPO pred launchem
...
| Soubor | Akce |
|--------|------|
| `app/(web)/ochrana-osobnich-udaju/page.tsx` | NOVY — kompletni kod vyse |
| `app/(web)/ochrana-osobnich-udaju/loading.tsx` | NOVY — viz P0-01 vzor |
| `app/(web)/ochrana-osobnich-udaju/error.tsx` | NOVY — viz P0-01 vzor |
| `app/sitemap.ts` | UPRAVIT — pridat do staticPages |

## Overeni

- [ ] Stranka se renderuje na /ochrana-osobnich-udaju bez chyb
- [ ] Metadata (title, description, openGraph, canonical) jsou spravne
- [ ] Breadcrumbs funguji vcetne JSON-LD
- [ ] Vsech 10 GDPR sekci je renderovano
- [ ] Tabulka ucelu zpracovani je citelna i na mobilu (horizontalni scroll)
- [ ] Kontakt na UOOU je spravny (Pplk. Sochorova 27, Praha 7)
- [ ] Vsechna prava subjektu dle cl. 15-21 GDPR jsou uvedena
- [ ] Odkaz na /zasady-cookies funguje (po P0-04)
