# Plan P0-01: Pravni stranky — Obchodni podminky
...
Vytvorit stranku `/obchodni-podminky` s plnymi obchodnimi podminkami. Zakonna povinnost pro e-shop dle zakona 634/1992 Sb. o ochrane spotrebitele a zakona 89/2012 Sb. (obcansky zakonik).

---

## Kroky implementace

### Krok 1: Vytvorit page.tsx

**Soubor:** `app/(web)/obchodni-podminky/page.tsx` (NOVY)
...
### Krok 2: Vytvorit loading.tsx

**Soubor:** `app/(web)/obchodni-podminky/loading.tsx` (NOVY)
...
### Krok 3: Vytvorit error.tsx

**Soubor:** `app/(web)/obchodni-podminky/error.tsx` (NOVY)
...
export default function Error() {
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
      <h1 className="text-2xl font-bold text-gray-900 mb-4">Chyba pri nacitani stranky</h1>
      <p className="text-gray-500">Zkuste to prosim pozdeji.</p>
    </div>
  );
}
```

### Krok 4: Pridat do sitemap.ts

**Soubor:** `app/sitemap.ts`

Pridat do pole `staticPages` (za radek 93 — za posledni statickou stranku `/kolik-stoji-moje-auto`):
...
- **Font:** Outfit (zdi se globalne z `app/layout.tsx`)
- **Barvy:** `text-gray-900` headings, `text-gray-600` body, `text-orange-500` linky — shodne s `jak-prodat-auto`
- **Tailwind prose:** `prose prose-gray` — shodne s obsahovymi strankami
- **Breadcrumbs:** Pouzit `<Breadcrumbs>` komponentu PRED content containerem (jako `o-nas`, `kontakt`)
- **Layout:** Stranka spadne do `app/(web)/layout.tsx` — automaticky dostane Navbar + Footer (dle subdomeny)
- **Server Component:** Zadny `"use client"` — staticka stranka, zadny JS na klientu

---

## Dulezite poznamky

- Text JE sablona odpovidajici ceske legislative — MUSI byt revidovany pravnikem pred launchem
- Placeholder `[DOPLNIT]` oznacuji mista vyzadujici realne firemni udaje (ICO, adresa, cislo vlozky OR)
- Po implementaci P1-07 (centralizace kontaktu) nahradit [DOPLNIT] za `companyInfo.*`
- Stranka pouziva `<Link>` pro interni odkazy (/reklamacni-rad, /ochrana-osobnich-udaju)
...
| Soubor | Akce |
|--------|------|
| `app/(web)/obchodni-podminky/page.tsx` | NOVY — kompletni kod vyse |
| `app/(web)/obchodni-podminky/loading.tsx` | NOVY |
| `app/(web)/obchodni-podminky/error.tsx` | NOVY |
| `app/sitemap.ts` | UPRAVIT — pridat do staticPages (za radek 93) |

## Overeni

- [ ] Stranka se renderuje na /obchodni-podminky bez chyb
- [ ] Metadata (title, description, openGraph, canonical) jsou spravne
- [ ] Breadcrumbs funguji a obsahuji JSON-LD BreadcrumbList
- [ ] JSON-LD WebPage schema je v `<head>`
- [ ] Vsech 11 sekci je renderovano s <h2> nadpisy
- [ ] Interni odkazy (/reklamacni-rad, /ochrana-osobnich-udaju) funguji
- [ ] Externi odkazy (COI, ODR platforma) maji target=_blank a rel=noopener
- [ ] Stranka je v sitemap.xml
- [ ] Stranka je vizualne konzistentni s ostatnimi obsahovymi strankami (jak-prodat-auto, o-nas)
- [ ] Na mobilu je text citelny a siroky dostatecne
