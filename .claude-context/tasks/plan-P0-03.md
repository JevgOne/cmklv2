# Plan P0-03: Pravni stranky — Reklamacni rad
...
Vytvorit stranku `/reklamacni-rad` se zakonnym reklamacnim radem pro e-shop s autodily. Povinnost dle zakona 634/1992 Sb. o ochrane spotrebitele.
...
**Soubor:** `app/(web)/reklamacni-rad/page.tsx` (NOVY)
...
### Krok 2: loading.tsx a error.tsx

Shodne s P0-01 — vytvorit oba soubory.

**Soubory:**
- `app/(web)/reklamacni-rad/loading.tsx` — viz P0-01 vzor
- `app/(web)/reklamacni-rad/error.tsx` — viz P0-01 vzor

### Krok 3: Pridat do sitemap.ts

**Soubor:** `app/sitemap.ts`

Pridat do pole `staticPages` (za radek 93):
...
- Shodne s P0-01 a P0-02: Outfit font, prose styling, Breadcrumbs, orange linky
- **Vizualni karty pro zarucni doby** — pouzivaji `not-prose` class pro vylouceni z Tailwind prose stylingu, barevne pozadi (green-50 pro nove, orange-50 pro pouzite) pro vizualni odliseni
- **Karta pro 30 dni** — modre pozadi (blue-50) pro vizualni zvyrazneni zakonne lhuty
- Interni odkazy na /obchodni-podminky pouzivaji `<Link>`
- Externi odkazy na COI a ODR maji `target="_blank"` a `rel="noopener noreferrer"`

---

## Dulezite poznamky

- Zarucni doby jsou spravne dle OZ: 24 mesicu nove, 12 mesicu pouzite
- Lhuta 30 dni pro vyrizeni je dle § 19 odst. 3 ZOS
- Kontakt na COI ADR oddeleni je realny a aktualni (Stepanska 567/15)
- Formular pro odstoupeni — bude doplnen po implementaci P0-09 (Return model)
- Text MUSI byt revidovany pravnikem pred launchem
...
| Soubor | Akce |
|--------|------|
| `app/(web)/reklamacni-rad/page.tsx` | NOVY — kompletni kod vyse |
| `app/(web)/reklamacni-rad/loading.tsx` | NOVY — viz P0-01 vzor |
| `app/(web)/reklamacni-rad/error.tsx` | NOVY — viz P0-01 vzor |
| `app/sitemap.ts` | UPRAVIT — pridat do staticPages |

## Overeni

- [ ] Stranka se renderuje na /reklamacni-rad bez chyb
- [ ] Metadata (title, description, openGraph, canonical) jsou spravne
- [ ] Breadcrumbs funguji vcetne JSON-LD
- [ ] Vizualni karty zarucnich dob (24m/12m) se zobrazuji spravne
- [ ] Karta "30 kalendarnich dni" je vizualne zvyraznena
- [ ] Zarucni doby pro nove (24m) i pouzite (12m) dily jsou uvedeny s odkazem na paragraf OZ
- [ ] Postup pro odstoupeni od smlouvy (14 dni) je popsany krok po kroku
- [ ] Kontakt na COI (Stepanska 567/15) a ODR platforma jsou spravne
- [ ] Odkaz na /obchodni-podminky funguje
- [ ] Stranka je v sitemap.xml
- [ ] Na mobilu jsou karty a tabulky citelne (grid cols-1 na sm)
